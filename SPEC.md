...
@SECURITY.md · History: git and the pull requests.
...
order they ship. Sequences Phases 2, 3 and 6 rather than replacing them, and
closes @DESIGN.md §10's Outstanding list.
...
undecided** — the seam is provider-agnostic and must stay that way. It is also
what gives the JSON-LD an `acquireLicensePage` to point at.
...
1. **No UI framework, and one sanctioned client script.** Zero client JS by
   ...
   (hover for a mouse, a still hold for a finger) and budgeted. Test-enforced.
2. **The supply-chain posture in @SECURITY.md holds**: pnpm only, frozen
   lockfile, exact pins, no unsanctioned lifecycle scripts, release cooldown on.
