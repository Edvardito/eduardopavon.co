...
with a two-face Typekit pairing — see Typography.
...
optically. _Loses to:_ the masthead, which is a solid ink band and is the reason
...
faithful reproduction, and this bug has already shipped once — a percentage
height against an aspect-ratio-derived box silently cropped every plate. _Loses
to:_ the viewport. A tall work at the common measure is taller than a screen,
and in the gallery it is allowed to be; seeing a work whole is the detail page's
job.
...
caps, and below that it stops reading as type and starts reading as texture. It
shipped once with every section label at 13px, which is what the floor exists to
prevent — no step the display face uses may fall under it, and that is
test-enforced (§10). The consequence is a real constraint, not a note: **this
face cannot do small furniture.** Anything that has to be small belongs to the
text face.
...
- **The space opening a cycle is at least twice the widest space inside one**,
  so a cycle boundary is felt without being announced. With the gallery reduced
  to one column this is the only thing carrying the cadence, so it is
...
**Gallery plates are not height-capped, and that is deliberate.** A cap was
built first and removed: it makes tall works narrower than short ones,
destroying the evenness the common measure exists for. The cost is that a tall
work exceeds the viewport in the gallery. No plate anywhere is capped: the
frontispiece that carried the one exception went when the hero did. The common
measure is a single token — the dial for trading magnificence against seeing a
work whole.
...
**What it was, and what the change cost.** This section first specified a
composed page: five slots of differing column spans and height caps, a facing
spread with a vertical drop, a mirrored pause. It was built, reviewed, and
replaced by the designer — works at differing sizes read as some being too
large, and one column reads as more magnificent. The horizontal composition is
gone, and with it the spread, the drop and the offsets. **Rhythm is now the
cadence's only instrument.** That is a real loss of editorial texture, taken
knowingly; do not reinstate the offsets because this section once described
them.
...
single column had already given up making. The page now goes straight from the
name to the contents to the work, and the first plate is simply the first plate.
...
- **On a plate, the glass is inside the work's bounds.** This reverses what this
  section first said. The original rule — edge only, painted entirely outside,
  never over the drawing — was derived from the photograph and from protecting
  faithful reproduction, and it was built that way. It read as a **drop
  shadow**, because everything outside a rectangle at low alpha does. The pane
  now sits **on** the work: contact line inset, rim drawn just inside the edge,
  and the light travelling across the surface. The cost is real and is the
  reason the first rule existed — a highlight now passes over the drawing, and
  the outermost hairline of every plate is overlaid. It is a light overlay,
  never a displacement: **the plate does not refract, so the work is never
  distorted**, and that is the part of the promise that does not bend.
...
- **The glazing is almost invisible, and that is correct.** Glass over a flat
  ...
  shadow. The first build drew it heavily enough to read as a mount; the alphas
  are now deliberately low. Raising them turns the glazing back into a border.
...
- **Nothing is painted outside the plate.** An outer cast shadow is exactly what
  made this read as furniture around a picture rather than as glass on one.
...
`Enmarcada` works was considered and rejected: semantically honest, but it makes
half the gallery inconsistent to encode a distinction belonging to the record,
not the object. Do not re-propose it.
...
- **The name is the glass.** The page blurs as it passes _behind_ it: a
  translucent ground with a backdrop blur on the title card itself. A blurred
  band below the whole masthead was built first and removed — under an opaque
  strip it blurred nothing but the content just emerging, which is the one place
  it should not. The masthead must therefore not be an isolated stacking
  context, or the backdrop root becomes the masthead itself and there is nothing
  behind it to sample.
...
over the work rather than as light on glass. **The glazing is the borders and
nothing else**: a contact line inset at the work's edge and the conic rim just
inside it, both static. Do not reintroduce a moving highlight on a plate.

**The magnifier — the one script.** A **foundation change**: invariant #1 was
"no UI framework, no client runtime, zero client JS"; it is now "zero client JS
except one justified progressive enhancement", amended with its rationale in
@SPEC.md and named in @CLAUDE.md.

The effect splits in two, which is what makes it affordable. **Magnification is
not a `backdrop-filter` problem** — that blurs what is behind an element and
cannot magnify. The zoom is a scaled copy: a lens whose background image is the
high-resolution source, `background-size` scaled up, `background-position`
derived from the cursor. **Glass realism is plain CSS**, from the shared glass
tokens. Both work everywhere.

**Refraction, and the distinction that makes it affordable.** The usual
liquid-glass recipe is `backdrop-filter: url(#svg-filter)` with
`feDisplacementMap`, and that part genuinely is Chromium-only — WebKit does not
support SVG filters in `backdrop-filter` and falls back to plain blur
(w3c/svgwg#1142 is open). **We do not need it.** `backdrop-filter` samples what
is _behind_ an element; the loupe already carries the magnified drawing as its
own layer, so the displacement runs through plain `filter`, which is not subject
to that gap. Refraction is therefore real rather than faked.

Two honest limits. `feImage` with an external reference is historically
unreliable outside Chromium, so where it does not resolve the lens degrades to
unrefracted magnification — complete and correct, just flatter; it is a
progressive enhancement, not a dependency. And **no chromatic aberration**:
splitting channels would fringe the drawing in hues the palette does not
contain. The dispersion stays in the rim, in `--color-pen`, where it is a
decision rather than an artifact.

The refracted view is drawn larger than the lens and clipped back to it, so
bending the rim pulls in real image instead of the transparency just outside the
circle — the first build smeared, which is what that looks like.
...
disc: a lens that arrives before its image reads as broken, not as loading. A
defocus-while-loading was built first and removed for exactly that reason. The
lens reads heavier than the glazing, being held in the hand rather than fixed
over the work, but from the same constants multiplied, so retuning the material
moves both objects together.
...
**Repo-size consequence, stated honestly:** artwork assets go from ~3.0 MB to
roughly 10 MB — a deliberate revision of a Phase 1 decision, recorded in
@SPEC.md's decisions log.
...
- **The magnifier's accessible equivalent.** The lens shipped before the detail
  page it points at; until that page exists, keyboard and touch users have only
  browser zoom. Deliberate, recorded, closed by Phase 3 — not inherited
  silently.
...
is taller than a laptop viewport and cannot be seen whole until the Phase 3
detail page exists. Narrower would fit every work on one screen and look
...
label names the obvious. Recommend keeping it; it is the seam where a second
section (a series, a year range) attaches, and removing it later is one line.

**Should `sin-titulo` be re-measured before Phase 3?** Carried from Phase 1: its
recorded ratio and its image's ratio disagree by 37% in either orientation. The
detail page will show the work full size beside those numbers, which is where a
wrong measurement becomes conspicuous. Recommend asking the artist to re-measure
before Phase 3 ships.

**Does the two-tier pipeline want its own skill?** Regenerating tiers, checking
budgets and verifying pairing is now multi-step, and it lives in `add-artwork`.
Recommend leaving it there until a second operation needs it — a skill per
procedure is cheaper than a skill per file.
